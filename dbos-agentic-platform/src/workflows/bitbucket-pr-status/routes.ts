...
const PrStatusRequestSchema = z.object({
  repo: z
    .string()
    .min(1)
    .regex(/^[^/]+\/[^/]+$/, "repo must be in workspace/slug format (e.g. atlassian/dt-proc)")
    .describe("Bitbucket repo in workspace/slug format"),
});

export function buildPrStatusRouter(client: DBOSClient): Router {
  const router = Router();

  router.post("/pr-status", async (req: Request, res: Response) => {
    const parsed = PrStatusRequestSchema.safeParse(req.body);
    if (!parsed.success) {
    ...
    const workflowId = `pr-status-${repo.replace("/", "-")}-${Date.now()}`;
    ...
    try {
      ...
      res.status(202).json({ workflowId, status: "ENQUEUED", pollUrl: `/workflow/pr-status/${workflowId}` });
    } catch (err) {
      res.status(500).json({ error: "enqueue_failed", message: (err as Error).message });
    }
  });

  router.get("/pr-status/:id", async (req: Request, res: Response) => {
    const { id } = req.params;
    try {
